...
def GetPass():
    ...
    cursor.execute("SELECT * FROM data where app = %s",(input('Input name of app: '),))
    y=cursor.fetchone()
    print('Email:',dec(y[2]),'\nPASS:',dec(y[3]))
...
def Update():
    ...
    q=input("what would u like to update 1, 2, 3 or any other key to exit: ")
    if (q)=='1':
...
def DelPass():
    ...
    if input('ARE YOU SURE YOU WANT TO PERMANENTLY DELETE THE PASSWORD?(y to continue)').lower()=='y':
...
while IsloggedOn :
    ...
    2-To retrieve password for an account
    3-To update password or other details for an account
    4-delete password for an account
    : ''')
    if ch=='1':
        SavePass()
